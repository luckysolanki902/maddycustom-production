...
export async function POST(request) { 
  try {
    ...
    // Check for SpecificCategoryVariant by pageSlug
    let variant = await SpecificCategoryVariant.findOne({ pageSlug: slug }).lean();
    
    if (variant) {
      console.info(`Found SpecificCategoryVariant: ${variant.name}`);
      const products = await Product.find({ specificCategoryVariant: variant._id }).lean();
      return NextResponse.json({ type: 'variant', variant, products }, { status: 200 });
    }

    // If no SpecificCategoryVariant found, check the Product model
    const product = await Product.findOne({ pageSlug: slug }).lean();
    
    if (product) {
      console.info(`Found Product: ${product.name}`);
      return NextResponse.json({ type: 'product', product }, { status: 200 });
    }
