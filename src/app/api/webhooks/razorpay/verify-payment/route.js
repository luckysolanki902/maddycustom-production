    ...
    if (event.event === 'payment.captured') {
      ...
      if (['allPaid', 'paidPartially'].includes(updatedOrder.paymentStatus)) {
        if (!updatedOrder.shiprocketOrderId && updatedOrder.deliveryStatus === 'pending') {
          console.log(`[${requestReceivedTime.toISOString()}] Creating Shiprocket order for Order ID: ${internalOrderId}`);
          ...
          console.log(`[${requestReceivedTime.toISOString()}] Shiprocket order already exists or delivery status is not pending.`);
        }
      }
