...
const templateSets = {
	home: ['Show me popular wrap designs','Help me pick a car accessory','Do you ship Pan India?','What makes your wraps unique?'],
	productList: ['Best selling pillar wraps?','Suggest a theme for my car','Show bike tank wraps (medium)','What’s your fastest shipping option?'],
	productDetail: ['Is this size right for my bike?','Any install tips for this wrap?','Care instructions after install?','Do you have similar styles?'],
	orderSuccess: ['Track my new order','Change shipping address (possible?)','Add another item to order?','When will packaging start?'],
	faq: ['Shipping time details','Wrap durability & care','Tank wrap sizing help','Order tracking support'],
	fallback: ['Where is my order?','Help me choose the right pillar wrap','How do I install a tank wrap?','What size wrap fits my bike?','How long does shipping take?','Suggest a fragrance for my car']
};
...
export default function SupportChatDialog({ open, onClose }) {
	...
	const { messages, loading, loadingHistory, pendingAssistant, retryLast, sendMessage, resetChat, invokeProductSearch } = useChatSession() || {};
	...
	const handleConfirmReset = async () => { await resetChat(); setShowTemplates(true); setConfirmOpen(false); };
			...
			>
				<div style={headerStyle}>
					...
					<div style={{ flex: 1 }}>
						...
						<div style={subtitleStyle}>{loadingHistory ? 'Loading conversation…' : 'Ask anything about your order, wraps & accessories'}</div>
					</div>
							...
							{showTemplates && (
								<div style={templatesWrap}>
									{initialTemplates.map(t => (
										<motion.button key={t} whileTap={{ scale: 0.94 }} onClick={() => handleTemplate(t)} style={templateBtnStyle}>{t}</motion.button>
									))}
								</div>
							)}
						</div>
				...
				<div style={inputBar}>
					<div style={{ display: 'flex', gap: 10 }}>
						<textarea
							...
							rows={1}
						/>
						<motion.button whileTap={{ scale: 0.94 }} disabled={!input.trim() || loading || pendingAssistant} onClick={handleSend} style={{ ...sendBtnStyle, opacity: !input.trim() || loading || pendingAssistant ? 0.6 : 1 }}>{loading || pendingAssistant ? '...' : 'Send'}</motion.button>
					</div>
...
const rootStyle = { position: 'fixed', bottom: 20, right: 24, zIndex: 3000, width: 'min(420px, 92vw)', height: '560px', display: 'flex', flexDirection: 'column', borderRadius: 28, boxShadow: '0 18px 48px -8px rgba(0,0,0,0.25)', background: '#ffffff', border: '1px solid rgba(45,45,45,0.12)', overflow: 'hidden', fontFamily: 'Jost, sans-serif' };
...
const messagesContainer = { flex: 1, overflowY: 'auto', padding: '18px 18px 12px', background: 'radial-gradient(circle at 80% 10%, rgba(45,45,45,0.04), transparent 60%)' };
...
const confirmOverlayWrap = { position: 'fixed', bottom: 90, right: 24, zIndex: 3100, width: 'min(420px, 92vw)', height: '560px', display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' };
